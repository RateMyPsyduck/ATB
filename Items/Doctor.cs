...
namespace ATB.Items
{
	public class Doctor : ModNPC
	{
        WeightedRandom<string> chat = new WeightedRandom<string>();

		public override void SetStaticDefaults() {
			Main.npcFrameCount[Type] = 2; // The amount of frames the NPC has

			NPCID.Sets.ExtraFramesCount[Type] = 2; // Generally for Town NPCs, but this is how the NPC does extra things such as sitting in a chair and talking to other NPCs.
		...
		// 	if (spawnInfo.Player.ZoneDirtLayerHeight && spawnInfo.Player.inventory.Any(item => item.type == ModContent.ItemType<Phaser>())) {
		// 		return 0.34f;
		// 	}
		...
		public override string GetChat() {
			...
			chat.Add("Please state the nature of the medical emergency.");
            chat.Add("I'm a Doctor not a doorstop.");
            chat.Add("Please state the nature of the medical emergency.");
            chat.Add("Please state the nature of the medical emergency.");
            chat.Add("Please state the nature of the medical emergency.");
            chat.Add("Please state the nature of the medical emergency.");
		...
		public override void SetChatButtons(ref string button, ref string button2) { // What the chat buttons are when you open up the chat UI
			button = "Heal"; //This is the key to the word "Shop"
            button2 = "Deactivate";
		}
		...
		public override void OnChatButtonClicked(bool firstButton, ref bool shop) {
			if (firstButton) {
                ...
                if(Main.LocalPlayer.statLife != Main.LocalPlayer.statLifeMax){
                    Main.LocalPlayer.HealEffect((Main.LocalPlayer.statLifeMax - Main.LocalPlayer.statLife), true);
                    Main.LocalPlayer.statLife = Main.LocalPlayer.statLifeMax;
                }
                else{
                    Main.npcChatText = "If there's nothing else, please remember to deactivate me before leaving.";
                }
			}
            else{
                NPC.life = 0;
