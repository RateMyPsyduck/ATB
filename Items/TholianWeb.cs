...
namespace ATB.Items
{
    class TholianWeb:ModProjectile
	{
		bool NCPFLAG = false;
		public int Timer;
		...
		public override void AI() {
			Player player = Main.player[Projectile.owner];
			Projectile.position = Projectile.position + Projectile.velocity * MOVE_DISTANCE;
		...
		*/
		private void SetLaserPosition(Player player) {
			...
			for (Distance = MOVE_DISTANCE; Distance <= 1500f; Distance += 5f) {
				...
				for(int i = 0; i < 200; i++){
					if(Collision.CheckAABBvLineCollision(Main.npc[i].getRect().Center(), Main.npc[i].getRect().Size(), Projectile.Center, Projectile.Center + Projectile.velocity * Distance, 22, ref point) && Main.npc[i].active == true){
						Flag = true;
				...
				if (!Collision.CanHitLine(Projectile.Center, 1, 1, start, 1, 1) || Flag == true) {
					Distance += 20f;
					//GivenName
					//Main.NewText(Main.npc[i].FullName, 150, 0, 0);
                    ...
                    //Main.NewText(Projectile.velocity.ToString(), 150, 0, 0);
					int num101 = (int)((Projectile.Center + Projectile.velocity * Distance).X / 16f) - 2;
					int num102 = (int)(((Projectile.Center + Projectile.velocity * Distance).X + (float)1) / 16f) + 1;
					int num103 = (int)((Projectile.Center + Projectile.velocity * Distance).Y / 16f) - 2;
					int num104 = (int)(((Projectile.Center + Projectile.velocity * Distance).Y + (float)1) / 16f) + 1;
					if (num101 < 0)
					{
						num101 = 0;
					}
